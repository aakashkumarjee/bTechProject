...
class WhereParser(Thread):
    ...
    def predict_operator(self, current_column_offset, next_column_offset):
        ...
        if(len(self.intersect(interval_offset, self.negation_keyword_offset)) >= 1) 
...
class Parser:
    ...
    def parse_sentence(self, sentence, stopwordsFilter=None):
        ...
        where_parser = WhereParser(new_where_phrase, tables_of_from, self.count_keywords, self.sum_keywords, self.average_keywords, self.max_keywords, self.min_keywords, self.greater_keywords,
                                   self.less_keywords, self.between_keywords, self.negation_keywords, self.junction_keywords, self.disjunction_keywords, self.database_dico, columns_of_values_of_where)
